...
def parse_llm_score(score_text):
    """Parse LLM's score from 'X/10' format and extract explanation."""
    try:
        score = int(score_text.split('/')[0])  # Extract the score
        explanation = score_text.split('/')[1].strip()  # Extract the explanation
        return score, explanation
    except Exception as e:
        print(f"Error parsing LLM score: {e}")
        return 0, "No explanation provided."  # Default to 0 and empty explanation
    ...
    if file_name.endswith('.csv'):
        ...
        try:
            ...
            relevancy_scores.append({
                ...
                'Explanation': result['explanation_candidate']
            })
        except Exception as e:
